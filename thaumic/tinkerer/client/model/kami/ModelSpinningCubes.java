...
extends ModelBase {
    ...
    public void renderSpinningCubes(int cubes, int repeat, int origRepeat) {
        ...
        float radiusBase = 0.7f;
        float radiusMod = 0.1f;
        double ticks = (double)ClientHelper.clientPlayer().field_70173_aa - 0.75 * (double)(origRepeat - repeat);
        ...
        for (int i = 0; i < cubes; ++i) {
            ...
            float radiusX = (float)((double)0.7f + (double)0.1f * Math.sin(ticks / 6.0));
            float radiusZ = (float)((double)0.7f + (double)0.1f * Math.cos(ticks / 6.0));
