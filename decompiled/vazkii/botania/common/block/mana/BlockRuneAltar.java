...
ILexiconable {
    ...
    public boolean func_149727_a(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9) {
        TileRuneAltar altar = (TileRuneAltar)par1World.func_147438_o(par2, par3, par4);
        ItemStack stack = par5EntityPlayer.func_71045_bC();
        if (par5EntityPlayer.func_70093_af()) {
            if (altar.manaToGet == 0) {
                for (int i = altar.func_70302_i_() - 1; i >= 0; --i) {
                    ItemStack stackAt = altar.func_70301_a(i);
    ...
    public boolean onUsedByWand(EntityPlayer player, ItemStack stack, World world, int x, int y, int z, int side) {
        ((TileRuneAltar)world.func_147438_o(x, y, z)).onWanded(player, stack);
        return true;
    }
