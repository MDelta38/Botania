...
extends EntityAIBase {
    ...
    public boolean func_75250_a() {
        ...
        ArrayList<IInventory> results = GolemHelper.getContainersWithRoom(this.theGolem.field_70170_p, this.theGolem, (byte)-1);
        if (results.size() == 0) {
            return false;
        }
        ...
        block0: for (IInventory te : results) {
            ...
            for (int side : GolemHelper.getMarkedSides(this.theGolem, (TileEntity)te, (byte)-1)) {
                if (!InventoryUtils.inventoryContains(te, this.theGolem.itemCarried, side, this.theGolem.checkOreDict(), this.theGolem.ignoreDamage(), this.theGolem.ignoreNBT())) continue;
