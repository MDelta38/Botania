...
implements IMessage,
IMessageHandler<PacketSyncScannedPhenomena, IMessage> {
    ...
    public PacketSyncScannedPhenomena(EntityPlayer player) {
        this.data = Thaumcraft.proxy.getScannedPhenomena().get(player.func_70005_c_());
    }

    @SideOnly(value=Side.CLIENT)
    public IMessage onMessage(PacketSyncScannedPhenomena message, MessageContext ctx) {
        for (String key : message.data) {
            Thaumcraft.proxy.getResearchManager().completeScannedPhenomena((EntityPlayer)Minecraft.func_71410_x().field_71439_g, key);
        }
        return null;
    }
}
