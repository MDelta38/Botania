...
extends ItemBlock {
    ...
    public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
        TileFluxScrubber tile;
        boolean ret = super.placeBlockAt(stack, player, world, x, y, z, side, hitX, hitY, hitZ, metadata);
        if (metadata == 14 && (tile = (TileFluxScrubber)world.func_147438_o(x, y, z)) != null && tile instanceof TileFluxScrubber) {
            tile.facing = ForgeDirection.getOrientation((int)side).getOpposite();
            tile.func_70296_d();
